...
describe CharacterWalletImporter do
  ...
  context "when character not found (404)" do
    ...
    before { expect(EveOnline::ESI::CharacterWallet).to receive(:new).with(character_id: character_id, token: access_token).and_raise(EveOnline::Exceptions::ResourceNotFound) }
