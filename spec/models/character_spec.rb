...
RSpec.describe Character, type: :model do
  ...
  it { expect(subject).to belong_to(:race).class_name("Eve::Race").optional(true) }

  it { expect(subject).to belong_to(:bloodline).class_name("Eve::Bloodline").optional(true) }

  it { expect(subject).to belong_to(:faction).class_name("Eve::Faction").optional(true) }

  it { expect(subject).to belong_to(:alliance).class_name("Eve::Alliance").optional(true) }

  it { expect(subject).to belong_to(:corporation).class_name("Eve::Corporation").optional(true) }

  it { expect(subject).to belong_to(:current_ship_type).class_name("Eve::Ship").optional(true) }

  it { expect(subject).to belong_to(:current_solar_system).class_name("Eve::System").optional(true) }

  it { expect(subject).to belong_to(:current_station).class_name("Eve::Station").optional(true) }

  it { expect(subject).to have_many(:character_scopes).dependent(:destroy) }

  it { expect(subject).to have_many(:loyalty_points).dependent(:destroy) }

  it { expect(subject).to have_many(:character_assets).dependent(:destroy) }

  it { expect(subject).to have_many(:character_implants).dependent(:destroy) }

  it { expect(subject).to have_many(:implants).through(:character_implants) }

  it { expect(subject).to have_many(:skillqueues).dependent(:destroy) }

  it { expect(subject).to have_many(:character_skills).dependent(:destroy) }

  it { expect(subject).to have_many(:character_corporation_histories).with_primary_key("character_id").dependent(:destroy) }

  it { expect(subject).to have_many(:character_mail_labels).dependent(:destroy) }

  it { expect(subject).to have_many(:standings).dependent(:destroy) }

  it { expect(subject).to have_many(:character_killmails).dependent(:destroy) }

  it { expect(subject).to have_many(:wallet_journals).dependent(:destroy) }

  it { expect(subject).to have_many(:wallet_transactions).dependent(:destroy) }

  it { expect(subject).to have_many(:character_blueprints).dependent(:destroy) }

  it { expect(subject).to have_many(:industry_jobs).dependent(:destroy) }

  it { expect(subject).to have_many(:character_orders).dependent(:destroy) }
  ...
  it { expect(subject).to delegate_method(:perception_bonus).to(:character_attributes) }

  it { expect(subject).to delegate_method(:memory_without_bonuses).to(:character_attributes) }

  it { expect(subject).to delegate_method(:memory_bonus).to(:character_attributes) }

  it { expect(subject).to delegate_method(:willpower_without_bonuses).to(:character_attributes) }

  it { expect(subject).to delegate_method(:willpower_bonus).to(:character_attributes) }

  it { expect(subject).to delegate_method(:intelligence_without_bonuses).to(:character_attributes) }

  it { expect(subject).to delegate_method(:intelligence_bonus).to(:character_attributes) }

  it { expect(subject).to delegate_method(:charisma_without_bonuses).to(:character_attributes) }

  it { expect(subject).to delegate_method(:charisma_bonus).to(:character_attributes) }

  it { expect(subject).to delegate_method(:science_skill).to(:character_skills_levels) }

  it { expect(subject).to delegate_method(:science_level).to(:character_skills_levels) }

  it { expect(subject).to delegate_method(:advanced_industry_skill).to(:character_skills_levels) }

  it { expect(subject).to delegate_method(:advanced_industry_level).to(:character_skills_levels) }
