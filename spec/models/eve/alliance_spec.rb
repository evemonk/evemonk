...
describe Eve::Alliance do
  ...
  describe "#formatted_corporations_count" do
    context "when number is 3" do
      subject { described_class.new(**attributes.merge(corporations_count: 111)) }

      specify { expect(subject.formatted_corporations_count).to eq("111") }
    end

    context "when number is 6" do
      subject { described_class.new(**attributes.merge(corporations_count: 111_222)) }
  ...
  describe "#formatted_characters_count" do
    context "when number is 3" do
      subject { described_class.new(**attributes.merge(characters_count: 111)) }

      specify { expect(subject.formatted_characters_count).to eq("111") }
    end

    context "when number is 6" do
      subject { described_class.new(**attributes.merge(characters_count: 111_222)) }
