...
RSpec.describe EveOnlineCallbackService do
  ...
  describe "#save!" do
    ...
    before { expect(subject).to receive(:assign_character_scope_attributes) }
    ...
    before { expect(subject).to receive(:character_scope).and_return(character_scope) }

    before { expect(character_scope).to receive(:save!) }
