...
describe Sitemap::Updater do
  describe "#update" do
    ...
    before { expect(FileUtils).to receive(:mkdir_p).with("/shared/public/") }

    let(:glob1) { double }

    before { expect(Dir).to receive(:glob).with("/shared/public/*.xml.gz").and_return(glob1) }
    ...
    before { expect(FileUtils).to receive(:cp).with(glob2, "/shared/public/") }
